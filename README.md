...
* **保存場所 (Data Store):** ローカルディレクトリ（デフォルト: `~/.clara/atoms`）に保存されます。
* **Frontmatter (YAML):** Atom ID、親Atom ID、作成日時、タグなどを記録。
* **Body:** ユーザーのプロンプトとAIの回答を記述。これらは `~~~~~~user` および `~~~~~~ai` ブロックによって分離されます。
...
### 4. LLM連携 (LLM Integration)

* `stdin` を介した直接的なCLI実行によりLLMと連携します（APIキー等はアプリ側で管理しません）。
